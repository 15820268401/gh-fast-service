...
/**
 * Created by hebang4 on 2017/8/10.
 * 日期工具类
 */
public class DateBaseUtil {
    ...
    public static boolean validateIsDate(String strTime) {
        ...
        if (date != null) {
            System.out.println("生成的日期:" + DateBaseUtil.parseDateToStr(date, DateBaseUtil.DATE_TIME_FORMAT_YYYY_MM_DD_HH_MI_SS, "--null--"));
            return true;
        }
        return false;
    }
     ...
     * 两个时间之间相差距离多少天
     *
     * @param beginDay 时间参数 1：
     * @param endDay 时间参数 2：
     * @return 相差天数
     */
    public static Long getDistanceDays(String beginDay, String endDay) throws Exception {
        DateFormat df = new SimpleDateFormat(DATE_TIME_FORMAT_YYYY_MM_DD_HH_MI_SS);
        Date one;
        Date two;
        long days = 0;
        try {
            one = df.parse(beginDay);
            two = df.parse(endDay);
            long time1 = one.getTime();
            long time2 = two.getTime();
            long diff;
